    ...
    if (type === "vc") {
        ...
        if (res.rows.length === 0) {
            await interaction.editReply(
                `✅ ${locale === "ja" ? `過去${days}日間のVC記録は見つかりませんでした。` : `No VC records found for the past ${days} days.`}`
            );
            return;
        }
        ...
        await interaction.editReply(
            `⏳ ${locale === "ja" ? `過去${days}日間のVCログを復元中... (${res.rows.length}件)` : `Restoring VC logs from the past ${days} days... (${res.rows.length} records)`}`
        );
        ...
        for (const session of res.rows) {
            ...
            const embedJoin = new EmbedBuilder()
                ...
                .setColor(0x00ff00)
                .setDescription(
                    `📥 [${locale === "ja" ? "復元" : "Restored"}] ${t("stat_vc_in", locale)}: **${channelName}**`
                )
                .setFooter({
                    text: locale === "ja" ? "過去ログスキャンによる復元" : "Restored via log scan",
                })
            ...
            if (session.leave_time) {
                ...
                const embedLeave = new EmbedBuilder()
                    ...
                    .setColor(0xff0000)
                    .setDescription(
                        `📤 [${locale === "ja" ? "復元" : "Restored"}] ${t("stat_vc_out", locale)}: **${channelName}**\n⌛ ${locale === "ja" ? "滞在時間" : "Duration"}: **${durationStr}**`
                    )
                    .setFooter({
                        text:
                            locale === "ja"
                                ? "過去ログスキャンによる復元"
                                : "Restored via log scan",
                    })
        ...
        await interaction.editReply(
            `✅ ${locale === "ja" ? `復元完了: **${recoveredCount}** 件のVCログをスレッドに再投稿しました。` : `Restoration complete: Re-posted **${recoveredCount}** VC logs to the thread.`}`
        );
    } else {
        ...
        for (const msg of sortedMessages) {
            ...
            for (const ng of ngWords) {
                if (ng.kind === "regex") {
                    ...
                    if (msg.content.includes(ng.word)) {
                        caught = true;
                        caughtWord = ng.word;
                    }
                }
        ...
        if (detectedCount === 0) {
            await interaction.editReply(
                `✅ ${locale === "ja" ? `過去${limit}件のメッセージにNGワードは見つかりませんでした。` : `No NG words found in the past ${limit} messages.`}`
            );
        } else {
            const report = detectedList.slice(0, 10).join("\n");
            const more =
                detectedList.length > 10
                    ? locale === "ja"
                        ? `\n...他 ${detectedList.length - 10} 件`
                        : `\n...and ${detectedList.length - 10} more`
                    : "";
            await interaction.editReply(
                `⚠️ **${detectedCount}${locale === "ja" ? "件" : " records"}** ${locale === "ja" ? "のNGワード候補が見つかりました。" : " of potential NG words found."}\n${locale === "ja" ? "ログチャンネルにも通知を送信しました。" : "Notifications sent to the log channel."}\n${report}${more}`
            );
        }
    }
}
