...
export async function sendLog(guild, type, payload, date = new Date(), options = {}) {
    try {
        const settingsRes = await dbQuery(
            "SELECT log_channel_id, ng_log_channel_id, mod_log_channel_id, color_log, color_ng, color_vc_join, color_vc_leave, branding_footer_text FROM settings WHERE guild_id = $1",
            [guild.id]
        );
                ...
                if (options.checkDuplicate) {
                    ...
                    if (messages && embed) {
                        const isDuplicate = messages.some((msg) => {
                            ...
                            const targetTime = embed.data.timestamp
                                ? new Date(embed.data.timestamp).getTime()
                                : null;
                            const msgTime = e.timestamp ? new Date(e.timestamp).getTime() : null;
                            const sameTime =
                                targetTime &&
                                msgTime &&
                                Math.floor(targetTime / 1000) === Math.floor(msgTime / 1000);
