...
## ✨ 機能表示の最適化と個別解放機能
- **機能表示の整理**: 混乱を防ぐため、既にリリース済みの「メンバー監査」をロードマップから除外しました。また、未リリースの「アンチレイド」や「自己紹介ゲート」は比較表から一旦除外し、ロードマップ専用の項目として整理しました。
- **プラン別機能リストの修正**: 各プランで現在本当に利用可能な機能のみがリストに表示されるよう、i18n辞書を修正しました（プロプラスにメンバー監査を明記、未実装機能を削除）。
- **個別解放機能（フィーチャーフラグ）の導入**: データベースに試験運用フラグを導入しました。これにより、正式リリース前の機能を特定のサーバー限定で先行してテスト・確認することが可能になりました。
