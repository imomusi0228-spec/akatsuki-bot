        ...
        try {
            ...
            switch (entry.action) {
                ...
                case AuditLogEvent.MemberRoleUpdate:
                    ...
                    if (added?.new?.length && flags.role_add) {
                        ...
                        fields.push({
                            name: "付与されたロール",
                            value: added.new.map((r) => r.name).join(", "),
                        });
                        ...
                        fields.push({
                            name: "剥奪されたロール",
                            value: removed.new.map((r) => r.name).join(", "),
                        });
                    }
                    break;
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    {
                        name: "実行者",
                        value: executor ? `<@${executor.id}> (${executor.tag})` : "不明",
                        inline: true,
                    },
                    {
                        name: "対象",
                        value: target?.id ? `<@${target.id}>` : target?.name || "不明",
                        inline: true,
                    },
