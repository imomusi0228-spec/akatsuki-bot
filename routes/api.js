...
export async function handleApiRoute(req, res, pathname, url) {
    ...
    if (pathname === "/api/button-roles" && method === "POST") {
        const body = await getBody();
        if (!body.guild || !body.channel_id || !body.buttons) return resJson({ ok: false, error: "Missing fields" }, 400);
        if (!await verifyGuild(body.guild)) return resJson({ ok: false, error: "Forbidden" }, 403);
        ...
        try {
            ...
            // Upsert DB
            if (body.id) {
                await dbQuery("UPDATE button_roles SET channel_id = $1, message_id = $2, content = $3, embed_title = $4, color = $5, buttons = $6, updated_at = NOW() WHERE id = $7 AND guild_id = $8",
                    [body.channel_id, messageId, body.content, body.embed_title, body.color, JSON.stringify(body.buttons), body.id, body.guild]);
            } else {
                await dbQuery("INSERT INTO button_roles (guild_id, channel_id, message_id, content, embed_title, color, buttons) VALUES ($1, $2, $3, $4, $5, $6, $7)",
                    [body.guild, body.channel_id, messageId, body.content, body.embed_title, body.color, JSON.stringify(body.buttons)]);
            }
