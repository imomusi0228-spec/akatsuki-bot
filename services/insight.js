...
async function generateAndSendInsight(guild, settings) {
    ...
    const queries = [];
    if (sections.includes("growth")) queries.push(dbQuery(`SELECT event_type, COUNT(*) as cnt FROM member_events WHERE guild_id = $1 AND created_at > NOW() - INTERVAL '7 days' GROUP BY event_type`, [guildId]));
    else queries.push(Promise.resolve({ rows: [] }));

    if (sections.includes("toxicity")) queries.push(dbQuery(`SELECT COUNT(*) as cnt FROM ng_logs WHERE guild_id = $1 AND created_at > NOW() - INTERVAL '7 days'`, [guildId]));
    else queries.push(Promise.resolve({ rows: [{ cnt: 0 }] }));

    if (sections.includes("vc")) queries.push(dbQuery(`SELECT COUNT(DISTINCT user_id) as cnt FROM vc_sessions WHERE guild_id = $1 AND join_time > NOW() - INTERVAL '7 days'`, [guildId]));
    else queries.push(Promise.resolve({ rows: [{ cnt: 0 }] }));
    ...
    if (sections.includes("growth")) {
        ...
        fields.push({ name: "直近7日の参加/離脱", value: `${joins}名 / ${leaves}名`, inline: true });
    }
