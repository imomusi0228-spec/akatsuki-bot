...
export async function execute(interaction) {
    ...
    if (!channelId) {
        return interaction.reply({
            content: locale === "ja" ? "❌ VCに入室した状態で実行してくださいわ。" : "❌ Please execute this command while you are in a voice channel.",
            ephemeral: true
        });
    }
    ...
    if (res.rows.length === 0) {
        return interaction.reply({
            content: locale === "ja" ? "❌ このVCは自動生成されたものではないため、操作できませんわ。" : "❌ This channel is not an auto-generated VC and cannot be modified.",
            ephemeral: true
        });
    }
    ...
    try {
        if (commandName === "vc-name") {
            ...
            return interaction.reply({
                content: locale === "ja" ? `✅ チャンネル名を **${newName}** に変更しましたわ。` : `✅ Changed channel name to **${newName}**.`,
                ephemeral: true
            });
        }
        ...
        if (commandName === "vc-limit") {
            ...
            const limitTxt = limit === 0 ? (locale === "ja" ? "無制限" : "Unlimited") : (locale === "ja" ? limit + "人" : limit);
            return interaction.reply({
                content: locale === "ja" ? `✅ 人数制限を **${limitTxt}** に変更しましたわ。` : `✅ Changed user limit to **${limitTxt}**.`,
                ephemeral: true
            });
        }
        ...
        if (commandName === "vc-owner") {
            ...
            if (targetUser.bot) {
                return interaction.reply({
                    content: locale === "ja" ? "❌ ロボットに主の座を譲るなんて、正気ですか？" : "❌ Are you seriously planning to transfer ownership to a bot?",
                    ephemeral: true
                });
            }
        ...
        return interaction.reply({
            content: locale === "ja" ? "❌ 申し訳ありません。技術的な問題で操作に失敗しましたわ。" : "❌ Sorry, an error occurred while processing your request.",
            ephemeral: true
        });
    }
}
