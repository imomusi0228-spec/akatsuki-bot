...
export async function runAutoRecoveryCheck() {
    try {
        ...
        const res = await dbQuery(
            "SELECT guild_id, antiraid_guard_level, last_raid_at FROM settings WHERE antiraid_auto_recovery_enabled = TRUE AND antiraid_guard_level > 0"
        );
        ...
        for (const s of guilds) {
            ...
            if (diffMins >= 30) {
                console.log(
                    `[AUTO-RECOVERY] Peace detected for ${diffMins.toFixed(1)} mins in guild ${s.guild_id}. Reverting to Normal mode.`
                );

                // 1. Update DB
                await dbQuery(
                    "UPDATE settings SET antiraid_guard_level = 0, updated_at = NOW() WHERE guild_id = $1",
                    [s.guild_id]
                );
                ...
                const guild =
                    client.guilds.cache.get(s.guild_id) ||
                    (await client.guilds.fetch(s.guild_id).catch(() => null));
                if (guild) {
                    try {
                        ...
                        if (guild.verificationLevel === VerificationLevel.VeryHigh) {
                            await guild.setVerificationLevel(
                                ...
                                "Iron Fortress: Auto-Recovery System Activated"
                            );
                        }
                        ...
                        const embed = new EmbedBuilder()
                            ...
                            .setDescription(
                                "一定時間の平穏が確認されたため、防衛モードを自律的に解除し「通常モード」へ復帰しました。"
                            )
                            .setColor(0x00ff00)
                            .setTimestamp();
                        ...
                        console.error(
                            `[AUTO-RECOVERY ERROR] Discord API fail for ${s.guild_id}:`,
                            e.message
                        );
                    }
                }
